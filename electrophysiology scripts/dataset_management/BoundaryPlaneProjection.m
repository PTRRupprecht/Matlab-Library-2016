...
% BW = roipoly;
BWsmooth = conv2(double(BW),fspecial('gaussian',[50 50], 50),'same');
...
hold on; contour(ZgridO,15,'Color',[0.9 0.9 0.8])

hold on; contour(ZgridO,[0:10:140],'Color',[1.0 1. 0.7])
...
for j = 1:size(xyz,1)
    if ~isnan(xyz(j,1))
        ...
        for j = 1:size(py,1);
            ww = [py(j,1:size(py,2)); px(j,1:size(py,2)); pz(j,1:size(py,2))];
            distanceGrid(j,1:size(py,2)) = sum((ww- repmat(pp',[1 size(py,2)])).^2);
        end

        minX = min(distanceGrid(:));
        [gx,gy] = find(distanceGrid == minX);

        hold on; plot(gx,gy,'.k','Color',[1 1 0],'MarkerSize',32)
        pause(0.1)
    end
end
